...
handle: the stored-claim identifier used by governance operations. The gap
shows up sharply in this paper's internal diagnostic
...
Upstream, aggregate extractor scores can authorize a bad candidate stream and
collapse the policy comparison. Downstream, memory-state metrics can expose
relevant content while failing to certify the identifier those governance
operations need. Recent memory benchmarks broaden what systems are tested on.
This paper narrows how policy-level credit gets assigned.
...
use the same storage substrate for compared policies.
...
runs only after an extractor-quality gate. Artifacts stay inspectable so
failure cases can be audited. The running case study is a lightweight
consolidation queue policy
...
the number of evidence sessions returned. It produces a stable negative result
for base CQ on evidence completeness. A later preregistered follow-up repairs
that specific failure by
...
Layer & Contribution\\
\midrule
Problem & Standard memory scores can pass while the policy lacks a stable claim
identifier.\\
PFLC diagnostic & PFLC checks whether that identifier is exposed, with the
...
returned evidence from rank effects and residual answer errors.\\
...
\claim{Claim 2: Standard memory metrics can pass while policy lookup fails}
...
PFLC asks whether the corresponding identifier is exposed, not just whether
retrieved text is semantically relevant.
The identity-PFLC gap is the memory-state counterpart of the extractor-gating
gap: the aggregate score looks good, but the policy still cannot name the
stored claim. The field survey shows when released benchmark artifacts do or do
not expose the per-question identifiers needed to replay such diagnostics.
