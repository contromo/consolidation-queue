...
32B-gated comparison reaches mixed/partial support limited to named
mechanisms.
...
outputs to quantify returned gold evidence ids and rank effects.
...
can look cluster-correct while the governable evidence handle is missing.
...
published, PFLC decomposes LoCoMo scores into returned gold evidence ids,
evidence rank or context saturation, and answer-generation residuals. This
replay does not certify that a memory policy can name its own governable
evidence handle. The internal canonical-id diagnostic tests that separate
question. Without per-question artifacts, the same decomposition is impossible,
which is why the artifact-blocked subset remains the headline survey gap.
