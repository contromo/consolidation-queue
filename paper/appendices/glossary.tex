...
cardinality-repair follow-up & Phase~Y & The LongMemEval follow-up that
  ...
  controlled-probe loss. \\
\addlinespace
LongMemEval v1 readout-cardinality probe & X.4 & The controlled-pilot
  externalization of the same-stream protocol to LongMemEval v1. \\
\addlinespace
...
resolution audit & CQR & The earlier canonical-id/query-resolution replay
  ...
  reuses. \\
\addlinespace
ADD/UPDATE/NOOP eager-write baseline & \splitcode{Mem0\allowbreak Lite}
  (repo); \splitcode{Mem0\allowbreak Write\allowbreak Policy\allowbreak Lite}
  (legacy paper label) & Narrow ADD/UPDATE/NOOP write-time baseline;
  \emph{not} an end-to-end reproduction of the production Mem0 system. Repo
  identifier kept stable for artifact compatibility; reader-facing body text
  uses the descriptive baseline name. \\
\addlinespace
