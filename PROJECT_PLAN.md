...
1. Write a focused Bucket B follow-up plan around the surviving countable mechanisms, especially forced contradiction and preference drift.
2. Inspect the noisy-vs-oracle gap and failure examples for scope contamination, useful pending memory, and memory poisoning before proposing any mechanism-specific extension.
3. Keep Bucket C abstention, `CQDatedContestation`, LongMemEval transfer, prompt changes, validator changes, and new mechanism families out of scope unless a future preregistered Bucket B follow-up explicitly justifies them.
