...
Status: completed locked noisy policy-comparison execution reached Bucket B on 2026-05-15, followed by the Bucket B mechanism audit. The 2026-05-14 32B primary gates unlocked under the local unlock probe's Bucket A branch, `docs/noisy_policy_comparison_preregistration.md` locked the extracted-candidate policy comparison, and both `default` and `scenario_conditioned` policy-comparison cells ran without preregistered aborts. CQ wins versus Reflection on forced contradiction and preference drift only, records no countable directional losses, ties `Mem0Lite` on all frozen sentinel primary metrics, and has no replicate contradictions. This is mechanism-local noisy support, not broad noisy CQ superiority. The mechanism audit attributes forced contradiction as clean contestation/demotion survival and preference drift as partial survival; scope contamination has real 32B residual defects but also lookup-alignment pressure, while useful pending memory and memory poisoning are unattributed nulls rather than proven extractor-floor convergence because their 32B component artifacts are perfect across the audited dimensions.
